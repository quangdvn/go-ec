-- Table structure for pre_go_acc_user_ 9999
--------------------------------------------
...
CREATE TABLE `pre_go_acc_user_9999` (
  ...
  `user_is_authentication` tinyint unsigned NOT NULL COMMENT 'Authentication status: 0-Not Authenticate 1-Authenticated' 
  ...
  KEY `idx_user_mobile` (`user mobile`),
  KEY `idx_user_email` (`user_email`),
  KEY `idx_user_state` (`user_state`),
  KEY `idx_user_is_authentication` (`user_is_authentication`)
) ENGINE= INnODB DEFAULT DEFAULT COLLATE=utf8mb4_0900_ai_ci COMMENT='pre_go_acc_user_9999'
...
CREATE TABLE `pre_go_acc_user_verify_9999` ( 
  ...
  `verify_hash_key` varchar (255) NOT NULL,
  `verify_type` int DEFAULT '1' COMMENT 'User state: 1-Email, 2-Phone', 
